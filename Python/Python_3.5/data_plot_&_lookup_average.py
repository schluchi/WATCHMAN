...
data_file = list()
x_average = [0,1,2,784.1156692504883, 914.9265975952148, 1092.727371215820, 1287.933818817139, 1461.324371337891, 1630.115879058838, 1714.067996978760,10,11] #list()
ax = list()
...
ax[0].plot(x_ideal, y, label='data ideal')
    ...
    for window in range(0,512):
        for channel in range(0,16):
            for sample in range(0,32):
                x_tmp = x_tmp + data_file[voltage][window][channel][sample] 
    x_average.append(x_tmp/(512*16*32))"""
ax[0].plot(x_average[range_min:range_max], y[range_min:range_max], ':*', label='data average')
p = np.polyfit(x_average[range_min:range_max], y[range_min:range_max], degree)
f = np.poly1d(p)
print(p)
ax[0].plot(x_ideal[2:], f(x_ideal[2:]), ':.', label='data fit python')
p_tmp = [-2.618530611858000, 0.007571756387560110, -0.000005249264563050564, 1.448035256861069e-9]
p = list()
for i in range(0,4):
    p.append(p_tmp[3-i])
f = np.poly1d(p)
print(p)
ax[0].plot(x_ideal[2:], f(x_ideal[2:]), ':.', label='data fit zynq')
...
f = np.poly1d(p)
x_lookup = np.linspace(0,2.5,2048)
y_lookup = f(x_lookup)
ax[1].plot(x_lookup, y_lookup, '.', label="difference fit")
